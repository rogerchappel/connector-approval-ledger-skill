...
test('duplicate ids fail the ledger audit',()=>{const a=auditLedger({entries:[{id:'a1',status:'proposed',connector:'email',summary:'draft recap'},{id:'a1',status:'rejected',connector:'crm',summary:'skip update'}]},new Date('2026-01-01T00:00:00Z'));assert.equal(a.valid,false);assert.match(a.results[0].issues.join(' '),/duplicate id a1/);assert.match(a.results[1].issues.join(' '),/duplicate id a1/);});
